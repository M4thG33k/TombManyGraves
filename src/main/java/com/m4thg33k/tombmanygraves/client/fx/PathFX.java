...
//Borrowed from Botania's Wisp FX
public class PathFX extends Particle {

    public static final ResourceLocation particles = new ResourceLocation("tombmanygraves:textures/misc/pathParticle.png");
    ...
    public PathFX(World world, double x, double y, double z, float size, float red, float green, float blue, boolean distanceLimit, boolean depthTest, float maxAge)
    {
        ...
        if (distanceLimit)
        {
            int visiableDistance = 50;
            if (!FMLClientHandler.instance().getClient().gameSettings.fancyGraphics)
            {
                visiableDistance = 25;
            }

            if (renderentity == null || renderentity.getDistance(posX, posY, posZ) > visiableDistance)
            {
                particleMaxAge = 0;
            }
        }
